...
# Import dependencies from server.py
...
def get_vector_store_for_file(file_id: str, text_chunks: list):
    ...
    # Save to disk with file_id namespace
...
class LLMServicer(llm_pb2_grpc.LLMServiceServicer):
    ...
    def Summarize(self, request, context):
        ...
        try:
            ...
            # Generate summary using same chain as server.py
    ...
    def ProcessDocument(self, request, context):
        ...
        try:
            ...
            # Extract text (same as /process-pdfs)
            raw_text = get_pdf_text_from_bytes(request.content)
            if not raw_text.strip():
    ...
    def Chat(self, request, context):
        ...
        try:
            ...
            # Retrieve relevant chunks (same as /ask endpoint)
            docs = db.similarity_search(request.question, k=3)
            context_text = "\n\n".join([d.page_content for d in docs])
            
            # Generate answer using same chain as server.py
